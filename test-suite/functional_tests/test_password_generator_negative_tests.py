...
@pytest.mark.parametrize(
    "invalid_length, expected_length",
    [
...
def test_negative_invalid_length_input(
    invalid_length, expected_length, page, password_elements
):
    ...
    page.wait_for_function(
        "document.querySelector('input[name=\"password\"]').value !== ''", timeout=5000
    )
    new_password = password_elements["password_field"].input_value()
    assert old_password != new_password, "New password wasn't generated."
    assert (
        str(len(new_password)) == expected_length
    ), f"Expected length {expected_length}, but got {len(new_password)}."
...
def test_last_checkbox_remains_checked(page, password_elements):
    ...
    selected_checkboxes = [
        name for name, checkbox in checkboxes.items() if checkbox.is_checked()
    ]
    ...
    for checkbox_name in selected_checkboxes[:-1]:  # Leave the last one checked
        ...
        assert not checkboxes[
            checkbox_name
        ].is_checked(), f"{checkbox_name.capitalize()} checkbox should be unchecked."
    ...
    assert checkboxes[
        last_checkbox_name
    ].is_checked(), f"{last_checkbox_name.capitalize()} checkbox should remain checked as the only selected option."
